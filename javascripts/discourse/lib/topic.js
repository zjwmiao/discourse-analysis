...
function onTopicListClick(ev) {
  ...
  while (
    !(target instanceof HTMLAnchorElement && target.classList.contains("title"))
  ) {
  ...
  const categories = [...mainLink.querySelectorAll(".badge-category__wrapper")]
...
export function reportTopicClick() {
  onNodeInserted("#main-outlet .topic-list .topic-list-body", (node) =>
    $(node).on("click", onTopicListClick)
  );
}
